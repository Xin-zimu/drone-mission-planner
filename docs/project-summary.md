...
The release is still simulation-only. It has no hardware telemetry, distributed networking, real-time guarantees, code signing, MAVLink/PX4, ROS 2, regulatory flight authorization, or fully three-dimensional A*. Collision handling is deterministic priority waiting rather than ORCA or Cooperative A*.
