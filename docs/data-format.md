...
Version 1.4 adds three-dimensional waypoints. Each drone persists a `waypoints` array next to the legacy `planned_path` point list. A waypoint is an object with `x`, `y`, `altitude`, `altitude_mode` (`msl` or `agl`), optional `speed`, `action` (`fly_to`, `hover`, `take_photo`, `scan`, `land`, or `return_to_launch`), `hold_seconds`, and optional `task_id` linking the waypoint to its mission task. Loading a file whose drones have a `planned_path` but no waypoints backfills waypoints using each drone's cruise altitude and terrain clearance; loading waypoints without a `planned_path` derives the point path from the waypoints. Saves always write both representations.
