...
The UI owns rendering. Domain planning coordinates are local metres; in georeferenced projects they are ENU (`x = east`, `y = north`, `z = up`) around the project origin, while legacy projects remain `local_only`. Screen-space transforms stay explicit in the UI layer, with north-up ENU mapped to Qt's downward-positive y only at render/input boundaries, so local metre coordinates are not silently interpreted as latitude/longitude. The georeference domain adapter owns WGS84/geodetic, ECEF and local topocentric transforms through PROJ/pyproj; import and export code call that adapter instead of applying fixed degree-to-metre factors. The 2.5D terrain view projects the same 2D mission coordinates with sampled terrain altitude; editing remains in the 2D view.
...
`planning/scheduling.py` owns the mission timeline: it propagates arrival, start, wait, finish and departure for a set of missions, validates the dependency graph, and reports hard-deadline violations and blocked missions. It is pure Python with no Qt dependency, deterministic, and the only implementation of the `start = max(arrival, earliest_start, predecessor floor)` rule — assignment scoring and the simulation consume its results instead of recomputing times. Drones keep one clock per aircraft, so a mission's arrival already includes everything that aircraft had to fly before it. Missions whose predecessors are missing, cancelled, failed, unscheduled or cyclic are reported as blocked with the reason, never silently scheduled.
