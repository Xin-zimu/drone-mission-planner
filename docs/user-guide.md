...
For a broad product walkthrough, open `examples/regional_emergency_demo.dmproj`. Start in point-task mode and choose **Planning → Auto assign missions** to inspect payload-, deadline-, terrain-, wind-, and relay-aware decisions. Run or step the simulation, inspect the 3D and Replay tabs, then export a report. Reset the project before trying **Planning → Plan all coverage areas**; the two sectors demonstrate priority ordering, horizontal/vertical scans, exclusion holes, and scarce-aircraft protection.
