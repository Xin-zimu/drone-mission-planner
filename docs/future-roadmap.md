...
| M9-lite | 最小高程导入 | 让 3D 地形有外部高程数据来源 | 下一步 |
| M13A | 三维航点模型 | 建立 `Waypoint` 和兼容迁移 | 计划中 |
| M13B | 真 3D 视图 | 展示地形、航线、空域和高度 | 计划中 |
...
- 用户能导入一个 `x,y,elevation` CSV 并在 Environment tab 看见高程范围。
- 2.5D 视图能显示导入后的地形起伏。
- 路线能耗和高度风险使用导入地形。
- 错误文件给出明确行号和字段名。
- `pytest / mypy / ruff` 通过。
...
- 新增 terrain import parser。
- 支持 `x,y,elevation`。
- 生成导入预览。
- 支持导入后替换 TerrainModel。
- 单元测试覆盖正常/缺列/非数值/重复点。
