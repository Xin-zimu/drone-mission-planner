...
> 后续实施入口（2026-09-06）：[v1.2「真实任务规划基础」详细实施计划](v1.2-real-mission-planning-plan.md)。新计划以航点统一、完整调度、全局多机优化、地理数据和目标飞控验证为主线；本文保留 M6–M20 的实施历史与当时的范围定义。
...
| M18 | 协同优化 | 时空预约、等待点、通道管制、中继机 | 已完成核心实现 |
| M19 | 可解释规划 | 分配解释、失败原因、优化建议 | 已完成核心实现 |
| M20 | 产品化 | 撤销/重做、自动保存、最近项目、设置页 | 已完成核心实现 |
...
- 新增 `planning/deconfliction.py`：`SpacetimeReservationTable` 按优先级顺序预约已分配航线（0.5 s 采样、含航点悬停后的到达时间），后处理航线在同刻距离小于安全间隔时在冲突航段起点顶点插入等待（hold_seconds，动作转 HOVER），并重检至解决；首航段冲突无法用到达悬停解决时明确跳过（记录为限制）。
- `Drone.role`（mission/relay）持久化；中继机不参与任务分配与覆盖规划（拒绝原因写入解释），仿真中在原位悬停（悬停能耗照计）。
- 通信图自动把中继机作为节点：测试验证中继机使远端作业机保持 2 跳连接。
- UI：自动分配后冲突与等待点写入 assignment_notes，随报告导出；偏 差说明：窄通道管制与跟随队形未实现。
...
- `ProjectService.change()` 提供最多 50 步的事务式项目快照，嵌套修改合并为一个操作；新增 Edit → Undo/Redo（`Ctrl+Z` / `Ctrl+Y`），撤销规划结果时同步丢弃失效仿真并刷新所有视图。
- `WorkspaceState` 在系统应用数据目录保存本地设置、去重并清理失效路径的最近项目列表，以及一个带来源/时间元数据的恢复快照。脏项目按可配置间隔自动保存；下次启动可选择恢复或丢弃，普通保存/主动丢弃会清除快照。
- Tools → Settings… 可配置自动保存开关、间隔、最近项目数量；Tools → Validation center… 汇总领域校验和二维路径/三维航点一致性。
- 项目保存改为同目录临时文件加原子替换，降低中断写入损坏风险；载入稀疏编号项目时按最大 ID 续号，避免对象 ID 重复。
- 修复多区域覆盖 UI 会被后续无可用无人机区域清空高优先级航线、Assignment weights 确认后读取不存在日志键导致崩溃、窗口销毁后 Qt 日志处理器仍引用已释放控件、删除对象遗留悬空引用的问题。
- Windows 构建固定使用 Python 3.12，隔离宿主 DLL 搜索路径并统一使用 PySide6 的 VC++ 14.44 运行库；成品支持自动退出启动自检。产品化核心能力已有自动化覆盖。窗口布局持久化和全局设备库仍可作为后续增强，不阻塞 M20 验收。

## 22. 当前收尾顺序

M6-M20 的核心实现均已完成，不再把这些阶段列为待开发任务。当前按发布风险排序：

1. 完成源码与 Windows 成品的 GUI 主流程验收。
2. 将 M20、回归修复和 Windows 打包修复保存为 checkpoint commit。
3. 更新可打印 Word/PDF 手册，并按需要制作截图或短视频。
4. 选择性补充产品体验：窗口布局持久化、跨项目设备库、报告回放关键帧、3D 底图纹理。
5. 选择性补充算法能力：按风向选择覆盖方向、窄通道管制、中继编队跟随。

## 23. 下一步可直接执行清单

### Task A：发布验收与 checkpoint

- Python 3.12 下运行完整 pytest、Ruff、mypy、compileall。已完成。
- 验证源码启动、项目打开、编辑、规划、仿真、回放、导出和恢复流程。已完成，并固化为端到端 GUI 回归测试。
- 构建 Windows EXE，并在隔离 DLL 环境中执行自动退出启动自检。已完成。
- 核对变更范围后创建 M20 checkpoint commit。随当前 checkpoint 完成。
...
- 持久化主窗口布局、dock 状态和最近使用视图。
- 把设备库提升为跨项目的全局库，同时保留项目内快照。
- 在 HTML 报告中加入回放关键帧。
- 在 3D 地形上显示已校准底图纹理。
...
- 真实飞控连接、在线遥测和 ROS 2/MAVLink 在线控制。
- 在线地图服务和完整 GIS 投影系统。
- 完整三维路径搜索；当前为二维 A*、三维航点和高度安全校验的组合。
- 代码签名和正式安装程序。
- 用 3D 视图取代 2D 主编辑器；当前两者并存。

## 25. 当前完成判断

M6-M20 核心路线已达到完成条件：

- 外部高程、GeoJSON/KML/CSV 任务数据可导入并持久化。
- 规划结果具有三维航点，仿真执行高度、速度和航点动作。
- 2D、2.5D、3D、回放和报告使用同一项目模型。
- 任务分配、覆盖、风险、协同避碰和中继能力均有自动化覆盖。
- 撤销/重做、自动保存、恢复、最近项目、设置、校验中心和原子保存可用。
- 旧项目迁移和完整测试套件保持通过。

当前剩余工作属于交付材料或可选增强，不阻塞 M20 核心验收。本轮核心开发与 GUI 主流程验收已经完成。
